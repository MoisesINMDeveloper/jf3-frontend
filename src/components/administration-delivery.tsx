...
const PanelAdminDelivery = () => {
  ...
  const fetchDeliveryZones = async () => {
    try {
      const fetchedZones = await getAllDeliveryOptions();
  ...
  const handleAddZone = async () => {
    try {
      const aliadoId = Number(Cookies.get("aliadoId")); // 👈 obtiene el aliado actual
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === "fee") {
      ...
      if (editZone) {
        setEditZone({ ...editZone, [name]: sanitizedValue });
      } else {
        setNewZone({ ...newZone, [name]: sanitizedValue });
      }
    } else {
      if (editZone) {
